...
import { getFlowModeFromModelId, type FlowModelTier, type FlowVideoMode } from "./modes";
import {
  flowFailureKind,
...
function requirePrompt(input: SubmitFlowTaskInput, mode: FlowVideoMode): string {
  ...
  if (!prompt && mode !== "remove-object") throw new Error("Prompt is required for this Google Flow mode");
  return prompt;
}
...
export async function submitFlowVideoTask(input: SubmitFlowTaskInput): Promise<FlowTaskRecord> {
  ...
  const tier = (input.parameters?.tier as FlowModelTier) || undefined;
  const duration = typeof input.parameters?.duration === "number" ? input.parameters.duration : undefined;
  const enableAudio = typeof input.parameters?.enableAudio === "boolean" ? input.parameters.enableAudio : undefined;
  const voice = typeof input.parameters?.voice === "string" ? input.parameters.voice : undefined;
  const cameraMotion = typeof input.parameters?.cameraMotion === "string" ? input.parameters.cameraMotion : undefined;
  const cameraPosition = typeof input.parameters?.cameraPosition === "string" ? input.parameters.cameraPosition : undefined;
  let upscaleSource: FlowVideoRecord | null = null;
  let extendSource: FlowVideoRecord | null = null;
  let editSource: FlowVideoRecord | null = null;
  let flowGenerationTrace: Record<string, unknown> | null = null;

  const needsVideoInput = mode === "upscale-video" || mode === "extend-video" || mode === "insert-object" || mode === "remove-object";
  if (needsVideoInput) {
    const video = firstString(collectInputVideos(input, ["video", "inputVideo", "videos"])[0], input.videos?.[0]);
    if (!video) throw new Error(`${mode} requires a Flow-generated video input`);
    const record = await findVideoMedia(video);
    if (!record?.mediaId) {
      throw new Error(`${mode} currently requires a video generated by Google Flow in this Banana project`);
    }
    if (mode === "upscale-video") upscaleSource = record;
    else if (mode === "extend-video") extendSource = record;
    else editSource = record;
  }

  const candidates = await listFlowRouteCandidates();
  const sourceAccountId = upscaleSource?.accountId ?? extendSource?.accountId ?? editSource?.accountId;
  const routedCandidates = sourceAccountId
    ? candidates.filter((candidate) => candidate.account?.id === sourceAccountId)
    : candidates;
  ...
  for (const candidate of routedCandidates) {
    ...
    try {
      ...
      const sourceRecord = upscaleSource ?? extendSource ?? editSource;
      const projectId = sourceRecord
        ? sourceRecord.projectId
        : await ensureProject(workflowId, input.workflowName, accountId, candidate.target);
      const { paygateTier: accountPaygateTier, raw: rawCredits } = await getCredits(candidate.target);
      const paygateTier = resolvePaygateTier(tier, accountPaygateTier);
      ...
      if (mode === "text-to-video") {
        submitResult = await generateTextToVideo({
          prompt,
          projectId,
          paygateTier,
          aspectRatio,
        ...
        flowGenerationTrace = {
          mode, prompt, aspectRatio, duration, enableAudio,
        ...
        flowGenerationTrace = {
          mode, prompt, aspectRatio, duration, voice,
          referenceMediaIds, referenceImageCount: images.length,
          submit: submitResult.trace,
        };
        ...
        flowGenerationTrace = {
          mode, prompt, aspectRatio, duration, startMediaId,
          submit: submitResult.trace,
        };
        ...
        flowGenerationTrace = {
          mode, prompt, seedPrompt, aspectRatio, duration,
          startMediaId, refMediaIds, seedMediaId,
          submit: submitResult.trace,
        };
        ...
        flowGenerationTrace = {
          mode, prompt, aspectRatio, duration, startMediaId, endMediaId,
          submit: submitResult.trace,
        };
      } else if (mode === "extend-video") {
        submitResult = await extendVideo({
          prompt,
          projectId,
          sourceMediaId: extendSource!.mediaId,
          paygateTier,
          aspectRatio,
          seed,
          bridgeTarget: candidate.target,
        ...
        flowGenerationTrace = {
          mode, prompt, aspectRatio,
          sourceMediaId: extendSource!.mediaId,
          submit: submitResult.trace,
        };
      } else if (mode === "camera-control") {
        const start = firstString(input.dynamicInputs?.startImage, input.dynamicInputs?.image, input.images?.[0]);
        if (!start) throw new Error("Camera Control requires a start frame image");
        const [startMediaId] = await uploadImages(input, projectId, [start], accountId, candidate.target);
        const endRaw = firstString(input.dynamicInputs?.endImage, input.images?.[1]);
        let endMediaId: string | undefined;
        if (endRaw) {
          [endMediaId] = await uploadImages(input, projectId, [endRaw], accountId, candidate.target);
        }
        submitResult = await generateCameraControlVideo({
          prompt,
          projectId,
          startMediaId,
          endMediaId,
        ...
        flowGenerationTrace = {
          mode, prompt, aspectRatio, duration,
          cameraMotion, cameraPosition,
          startMediaId, endMediaId,
          submit: submitResult.trace,
        };
      } else if (mode === "insert-object" || mode === "remove-object") {
        const region = (
          typeof input.parameters?.regionX === "number" &&
          typeof input.parameters?.regionY === "number"
        ) ? {
          x: input.parameters.regionX as number,
          y: input.parameters.regionY as number,
          width: (input.parameters.regionWidth as number) ?? 0.3,
          height: (input.parameters.regionHeight as number) ?? 0.3,
        } : undefined;
        submitResult = await editVideo({
          prompt,
          projectId,
          sourceMediaId: editSource!.mediaId,
          paygateTier,
          editType: mode === "insert-object" ? "insert" : "remove",
          region,
          bridgeTarget: candidate.target,
        });
        flowGenerationTrace = {
